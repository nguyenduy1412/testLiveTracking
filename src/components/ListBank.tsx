  ...
  return (
    <>
      <View>
        ...
        <TouchableOpacity style={styles.wrapperInput} onPress={() => actionSheetRef?.current?.show()}>
          <CommonText styles={styles.input} text={bankSelected !== '' ? bankSelected : 'Chọn ngân hàng'} />
        </TouchableOpacity>

        <ActionSheet ref={actionSheetRef} containerStyle={styles.actionSheet} onClose={onCloseActionSheet}>
          <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
            <TextInput style={styles.searchInput} placeholder="Tìm kiếm ngân hàng ..." placeholderTextColor={Colors.textSecondary} value={searchQuery} onChangeText={setSearchQuery} />
            {filteredBanks.length === 0 ? (
              <CommonText text="Không tìm thấy ngân hàng nào" styles={styles.textNotFound} />
            ) : (
              filteredBanks.map(bank => (
                <TouchableOpacity key={bank.id} style={styles.item} onPress={() => onSelectBank(bank)}>
